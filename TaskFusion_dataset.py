...
def prepare_data_path(dataset_path):
    filenames = os.listdir(dataset_path)#返回指定路径下的文件和文件夹列表
    data_dir = dataset_path
    data = glob.glob(os.path.join(data_dir, "*.bmp"))#返回所有匹配的文件路径列表
    data.extend(glob.glob(os.path.join(data_dir, "*.tif")))
    data.extend(glob.glob((os.path.join(data_dir, "*.jpg"))))
    data.extend(glob.glob((os.path.join(data_dir, "*.png"))))
    data.sort()
    filenames.sort()#对照片名字进行排序
...
class Fusion_dataset(Dataset):
    ...
    def __getitem__(self, index):
        if self.split == 'train':
            ...
            # image_vis = image_vis.astype(np.float32)/255
            ...
            image_inf = cv2.imread(ir_path, 0)
            label = np.array(Image.open(label_path))
            if len(image_vis.shape) == 2:
                image_vis = gray2rgb(image_vis)
            # if len(image_inf.shape) == 2:
            #     image_inf = gray2rgb(image_inf)
            image_vis = (
