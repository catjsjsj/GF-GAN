...
easy = 0 ; % easy = 1 means that the test runs are not time-consuming metrics, easy = 0 means that the test is time-consuming metrics
row_name1 = 'row1';
row_data1 = 'row2';
...
fileFolder=fullfile('D:\桌面\sss\SeAFusion-main\SeAFusionmain\test_imgs\21\ir'); % Folder where infrared images or visible images are located
dirOutput=dir(fullfile(fileFolder,'*.png')); % the suffix name of the source and fused images
fileNames = {dirOutput.name};
%自己加的
fileFolder1=fullfile('D:\桌面\消融实验\cbam\Fusion_results\21');
dirOutput1=dir(fullfile(fileFolder1,'*.png'));
fileNames1 = {dirOutput1.name};

[m, num] = size(fileNames);
ir_dir = 'D:\桌面\sss\SeAFusion-main\SeAFusionmain\test_imgs\21\ir'; % Folder where infrared images are located
vi_dir = 'D:\桌面\sss\SeAFusion-main\SeAFusionmain\test_imgs\21\vi'; % Folder where visible images are located
Fused_dir = 'D:\桌面\消融实验\cbam\Fusion_results\21'; % Folder where fused images are located
...
for j = 1:num
    ...
    fileName_Fusion = fullfile(Fused_dir, fileNames1{j});
...
file_name = './CABM.xls'; % File name for writing metrics to excel
if easy ==1
